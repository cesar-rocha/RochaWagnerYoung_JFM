...
#plt.contour(x,y,qpsi/(Ue*ke),cq[:2],colors='k')
plt.contourf(x,y,qpsi/(Ue*ke),cq,cmap=cmocean.cm.curl)
Q = plt.quiver(x[::dec,::dec],y[::dec,::dec],Fwx[::dec,::dec],Fwy[::dec,::dec],
                    scale=40,width=0.006)
qk = plt.quiverkey(Q, 0.25,.9, 4, r'$\mathbf{F}_w$', labelpos='E',
                   coordinates='figure')
...
cga = np.arange(-.75,.8,.05)
plt.contourf(x,y,Ga/Ga.max(),cga,vmin=-.75,vmax=.75,
