...
This solution dissipates much more geostrophic kinetic energy
than a waveless QG solution with the same parameters (Appendix B),
in which direct dissipation, $\varepsilon_\psi$, is about than $2\%$. This is because,
with waves, the quasi-geostrophic flow develops finer scales. The relative vorticity
is still populated by large coherent vortices that form via vortex-merger. But
$\lap\psi$ has more texture than than waveless QG relative vorticity,
...
smoother---but rougher than waveless QG---potential vorticity (Figure \eqref{pv-terms_turb}a).

\subsection{Varying dispersivity}
The energy exchange is strongly on dispersivity. Figure \ref{stats_turb_various}
summarizes results for the vertical wavelength $2\pi/m$ from 200 to 800 m,
which yield dispersivities from 0.25 to 4. (All other parameters are fixed.)
The initial conversion due to refraction, $\Gamma_r(t \times U_e k_e < 20)$,
increases slowly with $\hslash$ (Figure \ref{stats_turb_various}).

The advective conversion, $\Gamma_a$, dominates the total conversion for all
parameters. This terms is largest for the weakest dispersivity regime and
decays approximately exponentially with increasing dispersivity. Indeed, the initial
wave potential energy growth is sharper for weaker dispersivity. For $\hslash = 0.25$,
the wave potential energy peaks at $t\times U_e k_e\approx 25$, and then decays
(see blue curve in figure \ref{stats_turb_various}b).
In this case, dispersion is ineffective in counteracting the generation of small
scales by advection. And small-scale dissipation eventually damps this small scales.
The fine scales created by geostrophic advection also implies strong dissipation
of geostrophic kinetic energy.

The initial strong negative wave-vorticity correlation, a consequence of refraction, is equal
in all case. But reduction of this negative correlation, and eventual sign change,
occurs faster with decreasing dispersivity. The strong dispersion simulation
$\hslash> 1$ experience much slower decrease of correlation, which still
negativa at $t\times U_e k_e = 150$.
