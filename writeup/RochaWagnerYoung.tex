...
u + \ii v  &= \ee^{\ii (m z - f_0 t)}\,\phi -(\p_y-\ii\p_x)\psi\com\\
  \label{vertical_velocity}
  w &= \tfrac{1}{2 m} (\p_y-\ii\p_x)\ee^{\ii (m z - f_0 t)}\phi  + \text{cc}  \com\\
\label{pressure}
p &= \half f_0\lambda^2 (\p_y-\ii\p_x)\ee^{\ii (m z - f_0 t)}\phi
+\text{cc} + f_0 \psi \com\\
\label{buoyancy}
b &=  \tfrac{m}{2} f_0\lambda^2 (\p_x-\ii\p_y)\ee^{\ii (m z - f_0 t)}\phi + \text{cc} +
f_0 \psi_z  \per
\end{align}
Above $\phi = \phi(x,y,t)$ is a complex near-inertial back-rotated velocity,
$\lambda = \tfrac{N_0}{f_0\, m}$ is an intrinsic horizontal scale, and cc denotes
complex conjugate.

The balanced flow,
with streamfunction $\psi$, satisfies the wave-averaged inversion relationship
...
The wave model is similar to the \cite{young_benjelloul1997} equation. In other words,
