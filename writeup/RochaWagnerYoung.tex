...
Because geostrophic turbulence encumbers the flow of energy from forcing to
dissipative scales, fluxing energy back towards larger scales, poorly constrained
ageostrophic motions must play an important
role in closing the oceanic energy budget \citep{ferrari_wunsch2009}. Existing
studies suggest a panoply of mechanisms to account for the required forward
 flow of energy. These include
...
The unforced and inviscid NIW-QG model \eqref{macroturb}-\eqref{waves} conserves
NIW kinetic energy
\beq
\label{action}
...
\frac{\dd}{\dd t}\underbrace{\la\half |\grad \psi|^2\ra}_{\defn K_e} =
 - \Gamma \com
\eeq
thus proving the conservation of energy $E = K_e + P_w $. Above, $\Gamma =
...
wave terms, generalizes early ideas introduced by
 \cite{buhler_mcintyre1998} in setup that avoids spatial scale separation between
