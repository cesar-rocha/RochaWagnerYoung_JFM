  ...
  \begin{columns}[t]												% the [t] option aligns the column's content at the top
    ...
    \begin{column}{0.4\paperwidth}
      ...
      \begin{block}{The Xie \& Vanneste (2015) minimal model}
        ...
        single-mode near-inertial vertical structure, $u_w+\ii v_w  =
        \ee^{\ii (m z - f_0 t)} \phi(x,y,t)$, the Xie \& Vanneste \cite{XV15}
        coupled model reduces to
        \beq
        \label{macroturb}
        q_t + \sJ(\psi,q) = D_q\com
        \eeq
        with the \textit{wave-averaged} quasi-geostrophic potential vorticity
        (cf. \cite{WY15})
        \beq
        \label{qgpv}
        q = \lap \psi +
        ...
        and the D-terms represents an \textit{ad hoc} dissipation.

        \beq
        \label{action_density}
        \p_t \half |\phi|^2 + \sJ(\psi,\half|\phi|^2) + \diver \Ff_w = \text{dissipation}
        \per
        \eeq

        Above, $\Ff_w$ is the wave flux of kinetic energy density:

        \beq
        \label{Fw2}
        \Ff_w \defn \tfrac{\ii}{4}f_0\lambda^2\left(\phi\grad\phis-\phis\grad\phi\right) =
        f_0 \lambda^2\grad\Theta \times
        \half |\phi|^2\com
        \eeq

        where the equality above follows from use of a polar representation
        $\phi = |\phi|\ee^{\ii\Theta}$.
      ...
      \begin{alertblock}{The physics of stimulated imbalance}
                ...
                \, +\,\, \text{dissipation} \com
                \end{align}
                where $\Gamma_r+\Gamma_a$ is the wave-potential energy generation, which occurs
                at the expenses of geostrophic kinetic energy, $K_e \defn \half \la|\nabla\psi|^2\ra$,
                i.e.,
                \beq
                  \dot K_e = -(\Gamma_r+\Gamma_a) + \text{dissipative terms}\per
                \eeq

                \begin{figure}
                  \includegraphics[width=0.15\textwidth]{figs/Gammaa.pdf}
                  %\caption{...}
                \end{figure}
